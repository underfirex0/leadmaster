...
import {
  ...
  FileText, BarChart2, LogOut, Target, Shield, Crown
} from 'lucide-react'
...
export default function AdminNav({ name, email, pendingCount = 0 }: AdminNavProps) {
    ...
    <>
      ...
      <div className="p-5 border-b border-[rgba(0,0,0,0.05)] shrink-0">
        <Link href="/dashboard" className="flex items-center gap-2 mb-2 group">
          ...
          <span className="font-bold text-ink-1 text-[14px] tracking-tight">LeadScout</span>
        </Link>
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {NAV.map(({ href, label, icon: Icon, exact, badge }) => {
          ...
          const isPending = badge === 'pending' && pendingCount > 0
          return (
